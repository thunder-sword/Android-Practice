...
class ChessPiece(
    ...
    //获取当前棋子可前往的位置列表
    fun canToLocation(currentBoard: List<List<MutableList<ChessPiece>>>): List<Pair<Int, Int>> {
...
//        return allPosition
        ...
        fun isPathClear(board: List<List<MutableList<ChessPiece>>>, col: Int, row: Int): Boolean =
            board[col][row].all { !it.isFront } // 目标位置无正面棋子

        // 辅助函数：判断是否可吃子
        fun canCapture(board: List<List<MutableList<ChessPiece>>>, col: Int, row: Int): Boolean =
